...
fn sign_with_key(
    ...
    let pk_bytes = hex::decode(pk_hex).context(
        "Invalid AWP_PRIVATE_KEY hex — must be a valid hex string (with or without 0x prefix)",
    )?;
...
fn sign_with_wallet(
    ...
    if !token.is_empty() {
        ...
        log_debug!(
            "calling: {} sign-message --token *** --message <...>",
            wallet_bin.display()
        );
    } else if !agent_id.is_empty() {
        args.extend_from_slice(&["--agent", &agent_id]);
        log_debug!(
            "calling: {} sign-message --agent {} --message <...>",
            wallet_bin.display(),
            agent_id
        );
    } else {
        log_debug!(
            "calling: {} sign-message --message <...>",
            wallet_bin.display()
        );
    }
...
pub fn find_awp_wallet() -> Result<PathBuf> {
    ...
    for path_str in &candidates {
        ...
        log_debug!(
            "  Checking: {} — {}",
            path_str,
            if path.is_file() { "FOUND" } else { "not found" }
        );
        if path.is_file() {
    ...
    log_error!("awp-wallet not found. Searched PATH and: {:?}", candidates);
    bail!(
...
pub fn refresh_wallet_token() -> Result<String> {
    ...
    let mut args = vec!["unlock", "--duration", "86400", "--scope", "full", "--raw"];
    ...
    log_info!(
        "Wallet token refreshed successfully (length={})",
        token.len()
    );
