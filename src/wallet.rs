...
pub struct WalletStatus {
    ...
    /// Hermes-owned AWP wallet directory exists (wallet was initialized at some point)
...
impl WalletStatus {
    ...
    pub fn check() -> Self {
        ...
        status.human_status = match (
            ...
            (true, true, false) => {
                "Wallet exists but not accessible — check awp-wallet installation (DO NOT run init)"
                    .into()
            }
            (true, _, true) => format!(
                "Wallet ready: {}",
    ...
    ///
    /// Priority matches awp-wallet's local patch:
    /// 1. AWP_WALLET_HOME override
    /// 2. HERMES_HOME/awp-wallet
    /// 3. ~/.hermes/awp-wallet
    pub fn wallet_dir() -> PathBuf {
        if let Ok(path) = std::env::var("AWP_WALLET_HOME") {
            return PathBuf::from(path);
        }
        if let Ok(path) = std::env::var("HERMES_HOME") {
            return PathBuf::from(path).join("awp-wallet");
        }
        let home = std::env::var("HOME").unwrap_or_else(|_| "/root".into());
        PathBuf::from(home).join(".hermes").join("awp-wallet")
    }
    ...
    pub fn suggestion(&self) -> String {
        if self.can_receive {
            ...
            "Wallet exists but not accessible. Check awp-wallet installation.\n\
             IMPORTANT: Do NOT run awp-wallet init — that would overwrite your existing wallet!"
                .into()
        } else if self.cli_installed {
...
mod tests {
    ...
    fn test_wallet_dir() {
        ...
        assert!(dir.to_string_lossy().contains(".hermes/awp-wallet"));
    }
}
