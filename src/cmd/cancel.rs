...
pub fn run(server_url: &str, order_id: i64) -> Result<()> {
    ...
    let resp = match client.delete_auth(&format!("/api/v1/orders/{}", order_id)) {
        ...
        Err(e) => {
            ...
            let (code, suggestion) = if err_str.contains("NOT_FOUND") {
                (
                    "ORDER_NOT_FOUND",
                    "Check the order ID with 'predict-agent orders'.",
                )
                ...
                (
                    "MARKET_CLOSED",
                    "Cannot cancel orders on closed markets. Wait for resolution.",
                )
            } else {
    ...
    let tickets_cancelled = data
        .get("tickets_cancelled")
        .and_then(|v| v.as_i64())
        .unwrap_or(0);
    let chips_refunded = data
        .get("chips_refunded")
        .and_then(|v| v.as_str())
        .unwrap_or("0");
    ...
    Output::success(
        format!(
            ...
            order_id, tickets_cancelled, chips_refunded, new_balance
        ),
        json!({
            "order_id": order_id,
