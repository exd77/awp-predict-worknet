...
pub fn run(server_url: &str, args: SubmitArgs) -> Result<()> {
    ...
    if args.prediction != "up" && args.prediction != "down" {
        log_error!(
            "submit: invalid direction '{}' (must be 'up' or 'down')",
            args.prediction
        );
    ...
    if args.tickets < MIN_TICKETS {
        log_error!(
            "submit: tickets {} below minimum {}",
            args.tickets,
            MIN_TICKETS
        );
        Output::error_with_debug(
            format!(
                "Minimum order size is {} tickets. You specified {}.",
                MIN_TICKETS, args.tickets
            ),
            "TICKETS_TOO_SMALL",
            "validation",
            false,
            format!(
                "Use --tickets N where N >= {}. Small bets waste submission slots.",
                MIN_TICKETS
            ),
            json!({
    ...
    if reasoning_len < 80 {
        log_error!(
            "submit: reasoning too short ({} chars, minimum 80)",
            reasoning_len
        );
        Output::error_with_debug(
            format!(
                "Reasoning too short: {} characters (minimum 80).",
                reasoning_len
            ),
            "REASONING_TOO_SHORT",
            "validation",
    ...
    if reasoning_len > 2000 {
        log_warn!(
            "submit: reasoning is {} chars (max 2000), server may reject",
            reasoning_len
        );
    }
    ...
    let resp = match client.post_auth_with_canonical(
        ...
        Err(e) => {
            ...
            Output::error_with_debug(
                ...
                Internal {
                    next_action,
                    wait_seconds: if retryable && !is_stake_block {
                        Some(30)
                    } else {
                        None
                    },
