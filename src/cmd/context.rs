...
pub fn run(server_url: &str) -> Result<()> {
    ...
    let my_preds: Vec<String> = match &my_preds_result {
        ...
        Err(e) => {
            log_warn!(
                "context: failed to fetch my predictions (continuing): {}",
                e
            );
            vec![]
        }
    ...
    let (klines, klines_market_id) = if let Some(ref rec_id) = recommended_id {
        ...
        match client.get(&format!("/api/v1/markets/{}/klines", rec_id)) {
            Ok(resp) => {
                ...
                log_info!(
                    "context: received {} kline candles for {}",
                    candle_count,
                    rec_id
                );
            ...
            Err(e) => {
                log_warn!(
                    "context: failed to fetch klines for {} (continuing without): {}",
                    rec_id,
                    e
                );
                (None, None)
    ...
    let (action, reason, wait_seconds) = if submittable_ids.is_empty() {
        ...
        log_info!(
            "context: action=submit, {} markets available",
            submittable_ids.len()
        );
        (
            "submit",
