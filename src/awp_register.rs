...
pub fn check_registration(address: &str) -> Result<bool> {
    log_debug!(
        "awp_register: checking registration for {} on chain {}",
        address,
        CHAIN_ID
    );
    ...
    log_debug!(
        "awp_register: address.check result: registered={}, raw={}",
        registered,
        resp
    );
    Ok(registered)
...
pub fn ensure_registered(address: &str) -> Result<RegistrationResult> {
    ...
    log_debug!(
        "awp_register: EIP-712 typed data: {}",
        serde_json::to_string_pretty(&typed_data).unwrap_or_default()
    );
    ...
    log_info!(
        "awp_register: signature obtained ({}...{})",
    ...
    log_debug!(
        "awp_register: relay POST {} with body: {}",
        relay_url,
        relay_body
    );
    ...
    let relay_resp = client
        ...
        .send()
        .context(format!(
            "Failed to call registration relay at {}",
            relay_url
        ))?;
    ...
    log_info!(
        "awp_register: step 7/7 — polling for confirmation ({} attempts, {}s interval)...",
        POLL_ATTEMPTS,
        POLL_INTERVAL_SECS
    );
    for attempt in 0..POLL_ATTEMPTS {
        ...
        match awp_jsonrpc(
            ...
            Ok(refreshed) if is_registered(&refreshed) => {
                log_info!(
                    "awp_register: registration confirmed on attempt {}",
                    attempt + 1
                );
                return Ok(RegistrationResult {
                    registered: true,
            ...
            Ok(refreshed) => {
                log_debug!(
                    "awp_register: poll {}: not yet confirmed: {}",
                    attempt + 1,
                    refreshed
                );
...
fn awp_jsonrpc(client: &Client, method: &str, params: Value) -> Result<Value> {
    ...
    if let Some(err) = result.get("error") {
        ...
        log_error!(
            "awp_register: JSON-RPC {} error: code={}, message={}",
            method,
            code,
            msg
        );
    ...
    result.get("result").cloned().context(format!(
